...
#ifdef ESP32
  DEBUG_INFO("CFG", "Wait on sensor error actors: %ul sec", wait_on_Sensor_error_actor / 1000);
  DEBUG_INFO("CFG", "Wait on sensor error induction: %ul sec", wait_on_Sensor_error_induction / 1000);
  DEBUG_INFO("CFG", "Switch off actors on MQTT error: %d after %ul sec", StopOnMQTTError, (wait_on_error_mqtt / 1000));
...
bool saveConfig()
{
...
#ifdef ESP32
    DEBUG_INFO("CFG", "Actor #: %d Name: %s MQTT: %s PIN: %s INV: %d SW: %d", (i + 1), actors[i].getActorName().c_str(), actors[i].getActorTopic().c_str(), PinToString(actors[i].getPinActor()).c_str(), actors[i].getInverted(), actors[i].getActorSwitch());
#endif
  }
#ifdef ESP32
  DEBUG_INFO("CFG", TRENNLINIE);
  ...
  DEBUG_INFO("CFG", "Wait on sensor error actors: %ul sec", wait_on_Sensor_error_actor / 1000);
  DEBUG_INFO("CFG", "Wait on sensor error induction: %ul sec", wait_on_Sensor_error_induction / 1000);
  DEBUG_INFO("CFG", "Switch off actors on error enabled after %ul sec", (wait_on_error_mqtt / 1000));
  DEBUG_INFO("CFG", "Display: %d startPage: %d Fermenter: %d", useDisplay, startPage, useFerm);
