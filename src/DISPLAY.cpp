...
void cbpi4kettle_handlemqtt(unsigned char *payload)
{
  ...
  for (uint8_t i = 0; i < maxKettles; i++)
  {
    if (strlen(structKettles[i].id) == 0) // structKettle unbelegt
    {
      ...
      snprintf(sensorupdate, sizeof(sensorupdate), "%s%s", cbpi4sensor_topic, structKettles[i].sensor);
...
void cbpi4steps_handlemqtt(unsigned char *payload)
{
  ...
  for (uint8_t i = 0; i < stepsCounter; i++)
  {
    if (strcmp(structSteps[i].id, doc["id"]) == 0)
    {
      ...
      snprintf(structSteps[i].timer, sizeof(structSteps[i].timer), "%02d:%02d", minutes, 0);
      newStep = false;
      break;
    }
  }
  ...
  if (newStep == true)
  {
    ...
    snprintf(structSteps[stepsCounter].timer, sizeof(structSteps[stepsCounter].timer), "%02d:%02d", minutes, 0);
    stepsCounter++;
  }
  ...
  if (doc["status"] == "A")
  {
    ...
    else
    {
      if (props["Timer"].is<int>())
      {
        ...
        snprintf(currentStepRemain, sizeof(currentStepRemain), "%02d:%02d", minutes, 0);
        if (activePage == 0)
          nextion.writeStr(currentStepRemain_text, currentStepRemain);
        if (activePage == 1)
          nextion.writeStr(p1remain_text, currentStepRemain);
...
void cbpi4fermentersteps_handlemqtt(unsigned char *payload)
{
  ...
  {
    strlcpy(currentStepName, doc["name"] | "", maxStepSign);
    snprintf(currentStepRemain, sizeof(currentStepRemain), "%s", "");
  }
  else
  {
    snprintf(currentStepName, sizeof(currentStepName), "%s", "");
    snprintf(currentStepRemain, sizeof(currentStepRemain), "%s", "");
  }
}
