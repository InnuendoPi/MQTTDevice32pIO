...
void InnuNex::checkNex()
{
    ...
    if (_serial->available() > 2)
    {
        ...
        if (_start_char == 0x66) // change page
        {
            ...
            // Serial.printf("0x66 command: %d\n", _cmd1);
            _cmd1 = 0x66;

#ifdef ESP32
            if (debug)
                Serial.printf("\033[0;36m[%6lu][V][%s:%d] %s(): 0x66 (preinit) command: %d length: %d\033[0m\n", millis(), pathToFileName(__FILE__), __LINE__, __FUNCTION__, _cmd1, _len);
#endif
            ...
            if (_cmdFound)
            {
                ...
                cmdGroup = _cmd1;
                cmdLength = _len;
                char _ff = _serial->read(); // ff einlesen erforderlich
                // Serial.printf("0x66 command: %d length: %d ff: %x\n", _cmd1, _len, _ff);
                readCustomCommand(); // extern
            }
            return;
        } // if 0x66

        if (_start_char == 0x65) // Button
        {
            _len = _serial->read();
            _tmr1 = millis();
            _cmdFound = true;
            ...
            while (_serial->available() < _len)
            {
                if ((millis() - _tmr1) > 100UL) // 100UL
                {
                    _cmdFound = false;
                    break;
                }
            }
            // Serial.printf("0x65 command: %d length: %d found: %d\n", _cmd1, _len, _cmdFound);
            if (_cmdFound)
            {
                _cmd1 = _serial->read();
...
#endif
                // Serial.printf("0x65 command: %d length: %d\n", _cmd1, _len);
                cmdGroup = _cmd1;
                cmdLength = _len;
                readCustomCommand(); // extern
            }
            // else
            //     Serial.printf("0x66 command not found: %d length: %d\n", _cmd1, _len);
            return;
        } // if 0x65
    }
}
